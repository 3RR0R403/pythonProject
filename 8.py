...
class Human:
    ...
    def feed(self):
        self.pet_food -= self.pet.pet_feed
        if self.home.food <=0:
            self.shopping("food")
        else:
    ...
    def play_with_pet(self):
        self.gladness += self.pet.pet_gladness
    ...
    def days_indexes(self, day):
        ...
        pet_indexes = f"{self.pet} indexes"
        print(f"{pet_indexes:^50}", "\n")
        print(f"Pet satiety - {self.pet.satiety1}")
    def is_alive(self):
        if self.gladness < 0:
            print("Depression…")
            return False
...
class Pet:
    def __init__(self, pet_list):
        ...
        self.pet_feed = pet_list[self.pet]["food"]
